...
class AutoMetricsGenerator:
    ...
    def __init__(self, enable_default_metrics: bool = True):
        ...
        self._metrics: Dict[str, Dict[str, MetricStats]] = defaultdict(lambda: defaultdict(MetricStats))
        self._counters: Dict[str, Dict[str, int]] = defaultdict(lambda: defaultdict(int))
        self._histograms: Dict[str, List[Tuple[float, Dict[str, str], float]]] = defaultdict(list)
        
        # Rate calculation windows
        self._rate_windows: Dict[str, deque] = defaultdict(lambda: deque(maxlen=300))  # 5 minute window
    ...
    def _setup_default_extractors(self):
        ...
        # Error rate metrics
        self.add_extractor(MetricExtractor(
        ...
        self.add_extractor(MetricExtractor(
            ...
            value_extractor=lambda ctx: float(re.search(r"duration[_\s]*[=:]?\s*([0-9.]+)", 
                                                       str(ctx.get('message', ''))).group(1)) / 1000.0,
        ...
        self.add_extractor(MetricExtractor(
            ...
            value_extractor=lambda ctx: self._extract_numeric_value(ctx, "latency"),
    ...
    def process_log_record(self, log_record: Dict[str, Any]) -> List[MetricValue]:
        """
    ...
    def _extract_metric(self, extractor: MetricExtractor, log_record: Dict[str, Any]) -> List[MetricValue]:
    ...
    def _extract_memory_bytes(self, log_record: Dict[str, Any]) -> float:
        ...
        for pattern in patterns:
            ...
            if match:
                value = float(match.group(1))
                unit = match.group(2).lower() if len(match.groups()) > 1 else 'b'
    ...
    def _extract_numeric_value(self, log_record: Dict[str, Any], field_name: str) -> float:
    ...
    def _update_metric_storage(self, metric: MetricValue):
        ...
        elif 'histogram' in metric.name.lower() or 'duration' in metric.name or 'latency' in metric.name:
            # Histogram metric
            self._histograms[metric.name].append((metric.value, metric.labels, metric.timestamp))
            
            # Keep only recent values (last 1000 or 1 hour)
            cutoff_time = time.time() - 3600  # 1 hour
            self._histograms[metric.name] = [
                (v, l, t) for v, l, t in self._histograms[metric.name] 
                if t > cutoff_time
            ][-1000:]  # Keep last 1000 values
        
        else:
            # Gauge or general metric
            self._metrics[metric.name][labels_key].update(metric.value, metric.timestamp)
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            for metric_name, labels_data in self._metrics.items():
                if metric_name not in self._counters and metric_name not in self._histograms:
                    gauge_data = {}
            ...
            for metric_name, timestamps in self._rate_windows.items():
                ...
                recent_events = [t for t in timestamps if current_time - t <= 60]
        ...
        with self._lock:
            ...
            for metric_name, labels_data in self._counters.items():
                ...
                for labels_key, value in labels_data.items():
                    ...
                    lines.append(f"{metric_name}{labels_part} {value} {current_time}")
            ...
            for metric_name, values in self._histograms.items():
                ...
                # Count and sum
                lines.append(f"{metric_name}_count {len(numeric_values)} {current_time}")
                lines.append(f"{metric_name}_sum {sum(numeric_values)} {current_time}")
                
                # Quantiles
                for quantile in [0.5, 0.9, 0.95, 0.99]:
                    value = self._percentile(numeric_values, quantile * 100)
                    lines.append(f'{metric_name}{{quantile="{quantile}"}} {value} {current_time}')
            ...
            for metric_name, labels_data in self._metrics.items():
                if metric_name not in self._counters and metric_name not in self._histograms:
                    ...
                    for labels_key, stats in labels_data.items():
                        ...
                        labels_str = ','.join([f'{k}="{v}"' for k, v in labels_dict.items()])
                        labels_part = f"{{{labels_str}}}" if labels_str else ""
                        lines.append(f"{metric_name}{labels_part} {stats.last_value} {current_time}")
    ...
    def get_error_rate(self, time_window_seconds: int = 300) -> Dict[str, float]:
        ...
        with self._lock:
            ...
            for metric_name, labels_data in self._counters.items():
                if 'error' in metric_name.lower():
                    ...
                    error_rate = total_errors / time_window_seconds if time_window_seconds > 0 else 0
...
def create_web_service_metrics() -> AutoMetricsGenerator:
    ...
    generator.add_extractor(MetricExtractor(
        name="http_request_size_bytes",
        metric_type=MetricType.HISTOGRAM,
        pattern=r"request[_\s]*size[_\s]*[=:]?\s*([0-9.]+)",
        value_extractor=lambda ctx: generator._extract_numeric_value(ctx, "request_size"),
    ...
    generator.add_extractor(MetricExtractor(
        ...
        value_extractor=lambda ctx: generator._extract_numeric_value(ctx, "response_size"),
...
def create_database_metrics() -> AutoMetricsGenerator:
    ...
    generator.add_extractor(MetricExtractor(
        ...
        value_extractor=lambda ctx: generator._extract_numeric_value(ctx, "pool_size"),
    ...
    generator.add_extractor(MetricExtractor(
        ...
        value_extractor=lambda ctx: generator._extract_numeric_value(ctx, "rows_returned"),
