...
class MohFlowDjangoMiddleware(MiddlewareMixin):
    ...
    def __init__(self, get_response=None):
        ...
        self.log_level_mapping = config.get('log_level_mapping', {
    ...
    def _extract_request_context(self, request: HttpRequest) -> Dict[str, Any]:
        ...
        if self.log_user_context and hasattr(request, 'user') and request.user.is_authenticated:
        ...
        if (self.log_request_body and 
            context.get("content_type", "").startswith("application/json") and
    ...
    def _extract_response_context(self, response: HttpResponse, duration_ms: float) -> Dict[str, Any]:
        ...
        def wrapper(request, *args, **kwargs):
            ...
            if hasattr(request, 'mohflow_context'):
                request.mohflow_context.update({
                    'django_view': view_func.__name__,
                    **log_kwargs
                })
...
def setup_command_logging(logger: Any, command_name: str):
    ...
    command_logger = type(logger)(
        ...
        **{k: v for k, v in logger.__dict__.items() if k.startswith('_') == False}
    )
...
def configure_mohflow_django(logger: Any, **middleware_config) -> Dict[str, Any]:
    """
