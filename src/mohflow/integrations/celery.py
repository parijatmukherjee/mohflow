...
class MohFlowCeleryIntegration:
    ...
    def _task_prerun_handler(self, sender=None, task_id=None, task=None, args=None, kwargs=None, **kwds):
        """Handle task start."""
    ...
    def _task_postrun_handler(self, sender=None, task_id=None, task=None, args=None, kwargs=None, retval=None, state=None, **kwds):
        """Handle task completion."""
        end_time = time.time()
        
        # Get start time from context
        start_time = getattr(task, 'mohflow_context', {}).get('task_start_time', end_time)
    ...
    def _task_failure_handler(self, sender=None, task_id=None, exception=None, einfo=None, **kwds):
        """Handle task failure."""
        task_context = {
            "task_id": task_id,
    ...
    def _task_retry_handler(self, sender=None, task_id=None, reason=None, einfo=None, **kwds):
        """Handle task retry."""
        task_context = {
            "task_id": task_id,
            "task_name": sender.name if sender else "unknown",
...
def create_celery_logger(base_logger: Any, task_name: str) -> Any:
    ...
    task_logger = type(base_logger)(
        ...
        **{k: v for k, v in base_logger.__dict__.items() if not k.startswith('_')}
    )
...
def log_task_progress(logger: Any, task_id: str, current: int, total: int, message: str = None):
    """
    ...
    log_message = message or f"Task progress: {current}/{total} ({progress_percent:.1f}%)"
