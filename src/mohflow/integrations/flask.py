...
class MohFlowFlaskExtension:
    ...
    def init_app(self, app: Flask, logger: Any = None):
        ...
        self.log_request_body = app.config.get('MOHFLOW_LOG_REQUEST_BODY', False)
        self.log_response_body = app.config.get('MOHFLOW_LOG_RESPONSE_BODY', False)
        self.max_body_size = app.config.get('MOHFLOW_MAX_BODY_SIZE', 1024)
        self.exclude_paths = set(app.config.get('MOHFLOW_EXCLUDE_PATHS', []))
        self.exclude_status_codes = set(app.config.get('MOHFLOW_EXCLUDE_STATUS_CODES', []))
        self.log_level_mapping = app.config.get('MOHFLOW_LOG_LEVEL_MAPPING', {
            200: "info", 201: "info", 202: "info", 204: "info",
            400: "warning", 401: "warning", 403: "warning", 404: "warning",
            500: "error", 501: "error", 502: "error", 503: "error"
        })
    ...
    def _after_request(self, response):
        ...
        response_context = self._extract_response_context(response, duration_ms)
        
        # Log response
        if self.log_responses:
            log_level = self.log_level_mapping.get(response.status_code, "info")
    ...
    def _extract_request_context(self) -> Dict[str, Any]:
        ...
        context = {
            ...
            "flask_view_args": dict(request.view_args) if request.view_args else None
        }
        
        # Add request body if enabled
        if (self.log_request_body and 
            context.get("content_type", "").startswith("application/json")):
            try:
    ...
    def _extract_response_context(self, response, duration_ms: float) -> Dict[str, Any]:
...
def configure_mohflow_flask(app: Flask, logger: Any, **config) -> MohFlowFlaskExtension:
    """
    ...
    app.config.setdefault('MOHFLOW_LOG_REQUESTS', config.get('log_requests', True))
    app.config.setdefault('MOHFLOW_LOG_RESPONSES', config.get('log_responses', True))
    app.config.setdefault('MOHFLOW_LOG_REQUEST_BODY', config.get('log_request_body', False))
    app.config.setdefault('MOHFLOW_LOG_RESPONSE_BODY', config.get('log_response_body', False))
    app.config.setdefault('MOHFLOW_MAX_BODY_SIZE', config.get('max_body_size', 1024))
    app.config.setdefault('MOHFLOW_EXCLUDE_PATHS', config.get('exclude_paths', []))
    app.config.setdefault('MOHFLOW_EXCLUDE_STATUS_CODES', config.get('exclude_status_codes', []))
