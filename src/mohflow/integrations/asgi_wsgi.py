...
class MohFlowASGIMiddleware:
    ...
    async def __call__(self, scope: Dict[str, Any], receive: Callable, send: Callable):
        ...
        request_context = await self._extract_request_context(scope, receive, request_id)
        
        # Log incoming request
        if self.log_requests:
            with self.logger.request_context(request_id=request_id, **request_context):
                self.logger.info(
        ...
        try:
            ...
            if status_code and status_code not in self.exclude_status_codes:
                ...
                response_context = self._extract_response_context(response_data, duration_ms)
    ...
    async def _extract_request_context(self, scope: Dict[str, Any], receive: Callable, request_id: str) -> Dict[str, Any]:
        ...
        context["content_type"] = headers.get(b"content-type", b"").decode("utf-8")
        context["content_length"] = headers.get(b"content-length", b"").decode("utf-8")
    ...
    def _extract_response_context(self, response_data: Dict[str, Any], duration_ms: float) -> Dict[str, Any]:
        """Extract context from response data."""
        context = {
            "status_code": response_data["status_code"],
            "duration": duration_ms,
            "response_size": len(response_data["body"]) if response_data["body"] else None
        }
    ...
    def _get_client_ip(self, scope: Dict[str, Any], headers: Dict[bytes, bytes]) -> Optional[str]:
...
class MohFlowWSGIMiddleware:
    ...
    def _extract_request_context(self, environ: Dict[str, Any], request_id: str) -> Dict[str, Any]:
    ...
    def _extract_response_context(self, response_data: Dict[str, Any], body: bytes, duration_ms: float) -> Dict[str, Any]:
        """Extract context from response data."""
        # Parse status
        status_line = response_data["status"] or "500 Internal Server Error"
        status_code = int(status_line.split()[0])
...
def auto_setup_middleware(app, logger: Any, **config):
    ...
    # Generic ASGI app
        ...
        raise ValueError(f"Unable to auto-detect framework type for {app_type}")
...
def log_response_manually(logger: Any, request_id: str, method: str, path: str, 
