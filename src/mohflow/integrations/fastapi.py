...
class MohFlowFastAPIMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        custom_extractors: Optional[List[Callable[[Request], Dict[str, Any]]]] = None,
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            response_context = await self._extract_response_context(response, duration_ms)
    ...
    async def _extract_request_context(self, request: Request, request_id: str) -> Dict[str, Any]:
        ...
        if self.log_request_body and context.get("content_type", "").startswith("application/json"):
            try:
                body = await request.body()
                if len(body) <= self.max_body_size:
                    context["request_body"] = body.decode("utf-8")[:self.max_body_size]
            except Exception:
    ...
    async def _extract_response_context(self, response: Response, duration_ms: float) -> Dict[str, Any]:
        ...
        if (self.log_response_body and 
            ...
            try:
                ...
                if isinstance(body, bytes) and len(body) <= self.max_body_size:
                    context["response_body"] = body.decode("utf-8")[:self.max_body_size]
            except Exception:
...
def setup_fastapi_logging(
    ...
    **middleware_kwargs
    ...
    app.add_middleware(MohFlowFastAPIMiddleware, logger=logger, **middleware_kwargs)
    return app
