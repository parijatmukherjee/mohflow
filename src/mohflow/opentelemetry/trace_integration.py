...
class OpenTelemetryEnricher:
    ...
    def enrich_record(self, record: logging.LogRecord) -> logging.LogRecord:
        ...
        if self.include_service_info:
            if trace_context.service_name:
                setattr(record, "otel_service_name", trace_context.service_name)
            if trace_context.service_version:
                setattr(record, "otel_service_version", trace_context.service_version)
        
        # Add baggage items
        if self.include_baggage and trace_context.baggage:
            for key, value in trace_context.baggage.items():
                field_name = f"{self.baggage_prefix}{key}" if self.baggage_prefix else key
    ...
    def enrich_dict(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.include_service_info:
            ...
            if trace_context.service_version:
                enriched_data["otel_service_version"] = trace_context.service_version
...
def add_otel_context_to_logger(logger: logging.Logger, enricher: Optional[OpenTelemetryEnricher] = None):
