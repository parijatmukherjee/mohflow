...
def setup_otlp_exporter(
    ...
    try:
        ...
        endpoint = otlp_endpoint or os.getenv("OTEL_EXPORTER_OTLP_TRACES_ENDPOINT")
        
        if not endpoint:
            # Default to localhost
            endpoint = "http://localhost:4317" if not insecure else "http://localhost:4317"
...
def setup_exporter_from_env(
    ...
    if exporter_type == "console":
        return setup_console_exporter(service_name, service_version, resource_attributes)
