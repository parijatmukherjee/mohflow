...
class SlidingWindowCounter:
    ...
    def __init__(self, window_seconds: int = 60, bucket_count: int = 60):
        ...
        for i in range(bucket_count):
            ...
            self.bucket_timestamps.append(current_time - (bucket_count - i) * self.bucket_seconds)
    ...
    def _cleanup_old_buckets(self, current_time: float) -> None:
        ...
        while (self.bucket_timestamps and 
               self.bucket_timestamps[0] < cutoff_time):
...
class AdaptiveSampler:
    ...
    def should_sample(
        ...
        **kwargs: Any
        ...
        with self._lock:
            ...
            should_log = self._make_sampling_decision(sample_rate, level, message, **kwargs)
    ...
    def _check_rate_limits(self) -> SamplingResult:
        ...
        # Check burst limit
        if (self.config.burst_limit and 
            self.burst_counter and 
            self.burst_counter.get_count() >= self.config.burst_limit):
            return SamplingResult(
                should_log=False,
        ...
        # Check rate limit
        if (self.config.max_logs_per_second and 
            self.rate_limit_counter and
            self.rate_limit_counter.get_rate(1) >= self.config.max_logs_per_second):
            return SamplingResult(
                should_log=False,
                sample_rate_used=0.0,
    ...
    def _get_effective_sample_rate(self, level: str, component: Optional[str]) -> float:
        """Get the effective sample rate considering all factors."""
        base_rate = self._current_sample_rate
        
        # Check level-specific sampling
        if (self.config.level_sample_rates and 
            level in self.config.level_sample_rates):
            level_rate = self.config.level_sample_rates[level]
            base_rate = min(base_rate, level_rate)
        
        # Check component-specific sampling
        if (component and 
            self.config.component_sample_rates and
            component in self.config.component_sample_rates):
    ...
    def _make_sampling_decision(
        ...
        **kwargs: Any
        ...
        if self.config.strategy == SamplingStrategy.RANDOM:
            ...
            hash_input = f"{level}:{message or ''}:{kwargs.get('component', '')}"
    ...
    def _update_adaptive_sampling(self) -> None:
        ...
        if (current_time - self._last_adaptive_update >= 
            self.config.adaptive_window_seconds):
            
            current_rate = self.total_logs.get_rate(self.config.adaptive_window_seconds)
    ...
    def _maybe_cleanup(self) -> None:
        ...
        current_time = time.time()
        if (current_time - self._last_cleanup >= 
