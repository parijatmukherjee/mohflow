...
class OrjsonFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        # Serialize to JSON
        if HAS_ORJSON:
            return orjson.dumps(log_data, option=self._orjson_options).decode('utf-8')
        else:
            return json.dumps(
                log_data,
    ...
    def _format_timestamp(self, record: logging.LogRecord) -> Union[str, int, float]:
...
class StructuredFormatter(OrjsonFormatter):
    ...
    def __init__(self, **kwargs):
        ...
        kwargs.setdefault('static_fields', {
            'service': 'mohflow-app',
            'version': '1.0.0'
        })
