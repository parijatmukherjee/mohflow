...
class ComplianceReporter:
        ...
        if ComplianceStandard.HIPAA in self.enabled_standards:
            rules.extend([
                ComplianceRule(
                    ...
                    severity="critical",
                    pii_types={"medical_record", "ssn", "name", "date_birth"},
                    max_acceptable_level=PIILevel.NONE,
        ...
        if ComplianceStandard.CCPA in self.enabled_standards:
            rules.extend([
                ComplianceRule(
                    ...
                    severity="high",
                    pii_types={"name", "email", "phone", "address", "ip_address"},
                    max_acceptable_level=PIILevel.LOW,
    ...
    def check_compliance(self, pii_detections: Dict[str, PIIDetectionResult]) -> List[ComplianceViolation]:
        """
        ...
        for field_path, detection_result in pii_detections.items():
            for rule in self.compliance_rules:
                ...
                if any(pii_type in rule.pii_types for pii_type in detection_result.detected_types):
                    # Check if the PII level exceeds acceptable threshold
                    if self._violates_level_threshold(detection_result.level, rule.max_acceptable_level):
    ...
    def _violates_level_threshold(self, detected_level: PIILevel, max_acceptable: PIILevel) -> bool:
        ...
        return level_hierarchy[detected_level] > level_hierarchy[max_acceptable]
    ...
    def generate_compliance_report(self, 
        ...
        total_possible_violations = len(self.compliance_rules) * 100  # Theoretical max
        ...
        compliance_score = max(0, 100 - (weighted_violations / max(1, total_possible_violations // 10)) * 100)
        ...
        recommendations = self._generate_remediation_recommendations(relevant_violations)
        ...
        return {
            ...
            'compliance_summary': {
                ...
                'violations_by_standard': {k: len(v) for k, v in violations_by_standard.items()},
                'most_common_violation_types': dict(sorted(violation_types.items(), key=lambda x: x[1], reverse=True))
            },
            'detailed_violations': [
                {
                    'rule_id': v.rule_id,
    ...
    def _generate_remediation_recommendations(self, violations: List[ComplianceViolation]) -> List[Dict[str, Any]]:
        ...
        for action, action_violations in remediation_groups.items():
            ...
            recommendations.append({
                ...
                'standards_impacted': list(set(v.standard.value for v in action_violations)),
                'estimated_effort': self._estimate_remediation_effort(action, len(action_violations)),
    ...
    def _estimate_remediation_effort(self, action: str, violation_count: int) -> str:
        ...
        if violation_count > 50:
            ...
            effort = effort_levels[min(current_index + 1, len(effort_levels) - 1)]
    ...
    def _export_csv_report(self, report: Dict[str, Any]) -> str:
        ...
        writer.writerow(["Generated At", report['report_metadata']['generated_at']])
        writer.writerow(["Status", report['compliance_summary']['overall_status']])
        writer.writerow(["Score", report['compliance_summary']['compliance_score']])
        writer.writerow([])
        
        # Write violations
        writer.writerow(["Detailed Violations"])
        writer.writerow(["Rule ID", "Standard", "Severity", "Field Path", "PII Types", "Timestamp"])
    ...
    def _export_html_report(self, report: Dict[str, Any]) -> str:
        ...
        for severity, count in report['compliance_summary']['violations_by_severity'].items():
    ...
    def get_compliance_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'most_recent_violation': max(self.violations_log, key=lambda v: v.timestamp).timestamp.isoformat() if self.violations_log else None,
