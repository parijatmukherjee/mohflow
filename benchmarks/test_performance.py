...
def test_basic_logging_performance():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(
            f"✓ Basic Performance: {messages_per_second:,.0f} messages/second"
        )
...
def test_formatter_performance():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for name, (formatter_type, formatter_class) in formatters.items():
            ...
            print(
                f"✓ {name.title()} Formatter: {messages_per_second:,.0f} msg/sec"
            )

        # Verify FastJSONFormatter is faster than StructuredFormatter
        assert (
            results["fast"] >= results["structured"]
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            async def log_worker(worker_id, messages):
                for i in range(messages):
                    logger.info(
                        f"Async message {i} from worker {worker_id}",
                        worker_id=worker_id,
                        message_id=i,
                    )
            ...
            tasks = [
                log_worker(i, messages_per_worker) for i in range(workers)
            ]
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        def thread_worker(worker_id):
            ...
            for i in range(messages_per_thread):
                logger.info(
                    f"Thread message {i} from worker {worker_id}",
                    worker_id=worker_id,
                    message_id=i,
        ...
        print(
            f"✓ Threaded Performance: {total_messages_per_second:,.0f} msg/sec total"
        )
...
if __name__ == "__main__":
    ...
    if "formatters" in results and isinstance(results["formatters"], dict):
        fastest_formatter = max(
            results["formatters"].items(), key=lambda x: x[1]
        )
        print(
            f"Fastest Formatter: {fastest_formatter[0]} ({fastest_formatter[1]:,.0f} msg/sec)"
        )

    if (
        "memory" in results
        and isinstance(results["memory"], dict)
        and results["memory"]
    ):
        most_efficient = min(results["memory"].items(), key=lambda x: x[1])
        print(
            f"Most Memory Efficient: {most_efficient[0]} ({most_efficient[1]:.1f} MB)"
        )
