...
def quick_benchmark():
    ...
    print(
        f"📊 Testing {config.message_count:,} messages with {config.message_size} size"
    )
...
def comprehensive_benchmark():
    ...
    print(
        f"📊 Testing {config.message_count:,} messages - this may take a few minutes..."
    )
        ...
        try:
            ...
            for result in results:
                if "MohFlow" in result.name:
                    print(
                        f"  {result.name}: {result.messages_per_second:,.0f} msg/sec"
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--type",
        choices=["quick", "comprehensive", "sizes"],
        default="quick",
