...
def test_basic_otel_integration():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert (
            len(log_entries) >= 2
        ), f"Should have at least 2 log entries, got {len(log_entries)}"

        # Check for trace context in traced logs
        traced_entries = [
            entry for entry in log_entries if "trace_id" in entry
        ]
        assert (
            len(traced_entries) >= 2
        ), f"Should have at least 2 traced entries, got {len(traced_entries)}"
        ...
        assert (
            len(traced_entry["trace_id"]) == 32
        ), "Trace ID should be 32 hex chars"
        assert (
            len(traced_entry["span_id"]) == 16
...
def test_trace_context_extraction():
    ...
    assert (
        context_empty.trace_id is None
    ), "Should have no trace context without span"
    ...
    with tracer.start_as_current_span("context_test") as span:
        ...
        assert (
            context_with_span.trace_id is not None
        ), "Should have trace ID with active span"
        assert (
            context_with_span.span_id is not None
        ), "Should have span ID with active span"
...
def test_otel_enricher():
    ...
    enricher = OpenTelemetryEnricher(
        include_trace_id=True, include_span_id=True, include_baggage=True
    )
    ...
    assert (
        "trace_id" not in enriched_empty
    ), "Should not add trace_id without span"
    ...
    with tracer.start_as_current_span("enricher_test"):
        ...
        assert (
            "trace_id" in enriched_with_span
        ), "Should add trace_id with span"
        assert "span_id" in enriched_with_span, "Should add span_id with span"
        assert (
            enriched_with_span["message"] == "test"
        ), "Should preserve original fields"
        assert (
            enriched_with_span["user_id"] == 123
        ), "Should preserve original fields"
...
def test_logger_factory_methods():
    ...
    assert (
        logger_tracing.enable_otel
    ), "with_tracing should enable OpenTelemetry"
    assert (
        logger_tracing.otel_service_version == "2.0.0"
    ), "Should set service version"

    # Test microservice factory
    logger_micro = MohflowLogger.microservice(
        service_name="test_microservice", console_logging=False
    )

    assert logger_micro.enable_otel, "microservice should enable OpenTelemetry"
    assert (
        logger_micro.formatter_type == "production"
    ), "Should use production formatter"
    assert (
        logger_micro.otel_exporter_type == "otlp"
    ), "Should use OTLP exporter"

    # Test cloud_native factory
    logger_cloud = MohflowLogger.cloud_native(
        service_name="test_cloud", console_logging=False
    )
...
def test_trace_correlation():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert (
            len(log_entries) >= 4
        ), f"Should have at least 4 log entries, got {len(log_entries)}"
        ...
        assert (
            len(set(trace_ids)) == 1
        ), "All entries should have same trace_id"

        # Parent and child spans should have different span_ids
        span_ids = [
            entry.get("span_id") for entry in log_entries if "span_id" in entry
        ]
        assert (
            len(set(span_ids)) >= 2
        ), "Should have at least 2 different span_ids"

        print(
            f"✓ Trace correlation working - {len(log_entries)} messages correlated"
        )
        return True
...
def test_performance_with_tracing():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(
            f"✓ OpenTelemetry performance: {messages_per_second:,.0f} msg/sec"
        )
        ...
        trace_coverage = (
            len(traced_entries) / len(log_entries) if log_entries else 0
        )

        print(
            f"✓ Trace coverage: {trace_coverage*100:.1f}% ({len(traced_entries)}/{len(log_entries)})"
        )
