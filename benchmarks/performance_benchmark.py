...
class PerformanceBenchmark:
    ...
    def __init__(self, config: BenchmarkConfig = None):
        ...
        self.messages = {
            ...
            "large": f"Large message "
            + "x" * 500
            + f" with extensive context: {json.dumps({'user_id': 12345, 'session_id': 'abc123', 'request_data': {'path': '/api/users', 'method': 'GET', 'headers': {'user-agent': 'Mozilla/5.0'}, 'query_params': {'page': 1, 'limit': 50}}, 'response_time': 123.45, 'status_code': 200})}",
        }
    ...
    def _run_logging_benchmark(
        self, name: str, log_func: Callable, cleanup: Callable = None
    ) -> BenchmarkResult:
        ...
        success_rate = (
            self.config.message_count - errors
    ...
    def _run_async_logging_benchmark(
        ...
        async def run_benchmark():
            ...
            tasks_per_worker = (
                self.config.message_count // self.config.async_task_count
            )
    ...
    def _run_threading_benchmark(
        self, name: str, log_func: Callable
    ) -> BenchmarkResult:
        ...
        messages_per_thread = (
            self.config.message_count // self.config.thread_count
        )
        ...
        messages_per_second = self.config.message_count / duration
        success_rate = (
            self.config.message_count - total_errors
        ) / self.config.message_count
    ...
    def run_all_benchmarks(self) -> List[BenchmarkResult]:
        ...
        print(
            f"Configuration: {self.config.message_count:,} messages, {self.config.message_size} size"
        )
        print("=" * 70)
    ...
    def print_results(self):
        ...
        sorted_results = sorted(
            self.results, key=lambda r: r.messages_per_second, reverse=True
        )

        # Print header
        print(
            f"{'Library':<20} {'Msg/Sec':<12} {'Memory (MB)':<12} {'Avg Lat (μs)':<12} {'P99 Lat (μs)':<12} {'Success %':<10}"
        )
        print("-" * 100)
        ...
        for result in sorted_results:
            print(
                f"{result.name:<20} {result.messages_per_second:>11,.0f} "
        ...
        print(
            f"🏆 Fastest: {fastest.name} ({fastest.messages_per_second:,.0f} msg/sec)"
        )
        ...
        if mohflow_results:
            ...
            if best_mohflow != fastest:
                performance_gap = (
                    fastest.messages_per_second
                    / best_mohflow.messages_per_second
                )
                print(
                    f"📊 MohFlow Performance: {performance_gap:.2f}x slower than fastest"
                )
            else:
        ...
        print(
            f"💾 Most Memory Efficient: {most_efficient.name} ({most_efficient.memory_mb:.1f} MB)"
        )

        # Latency analysis
        lowest_latency = min(sorted_results, key=lambda r: r.avg_latency_us)
        print(
            f"⚡ Lowest Latency: {lowest_latency.name} ({lowest_latency.avg_latency_us:.1f} μs avg)"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="MohFlow Performance Benchmark Suite"
    )
    parser.add_argument(
        "--messages",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Number of threads for threading tests",
    )
    parser.add_argument(
        "--async-tasks", type=int, default=1000, help="Number of async tasks"
    )
    parser.add_argument(
        "--message-size",
        choices=["small", "medium", "large"],
        default="medium",
        help="Size of log messages",
    )
    parser.add_argument(
        "--no-context", action="store_true", help="Disable context data"
    )
    parser.add_argument("--save", type=str, help="Save results to file")
    parser.add_argument(
        "--warmup", type=int, default=10_000, help="Warmup message count"
    )
